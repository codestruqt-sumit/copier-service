...
The mode is picked **at boot**, from exactly one place:

```
COPIER_MODE = web|api        # in .env (web if unset). Change it -> restart to apply.
```

Env-only by decision (2026-08-31): one source of truth, no hidden precedence. A mode change
is restart-applied: the gateway is constructed once at boot, so the transport can never swap
mid-run (and never mid-order). The dashboard and /oauth/tradovate display the active mode
read-only.
